	...
	function finderTreeWrapped($compile, RecursionHelper) {
		return {
			...
			template: '<div data-ng-show="data.displayed" class="finder-tree">' +
				...
				'<i class="fa fa-user-o padding-small no-vertical-padding para-large vertical-align-middle" aria-hidden="true"></i>' +
				'<span class="ft_span_text vertical-align-middle">{{dir.name}}</span>' +
				'<i class="fa fa-chevron-right vertical-align-middle" aria-hidden="true"></i></li>' +
			...
			compile: function (element) {
				return RecursionHelper.compile(element, function (scope, iElement, iAttrs, controller, transcludeFn) {
					...
					if(scope.fileimg)
						fileImg = scope.fileimg;
					if(scope.folderimg)
						folderImg = scope.folderimg;

					$('.ft_folder').css('background-image', folderImg);
					...
					scope.displayNext = function (index) {
						...
						if(scope.data.dirs[index].files && !scope.data.dirs[index].dirs)
							var itemNumber = scope.data.dirs[index].files.length;
						else if(scope.data.dirs[index].dirs && !scope.data.dirs[index].files){
							var itemNumber = scope.data.dirs[index].dirs.length;
						}
						else{
						...
						controller.$render();
						$('.ft_file').css('background-image', fileImg);
					};
					...
					scope.select = function (file, index) {
						...
						file.path = path;
						// Number of item in current dir
						if(scope.data.files && !scope.data.dirs)
							var itemNumber = scope.data.files.length;
						else if(scope.data.dirs && !scope.data.files)
							var itemNumber = scope.data.dirs.length;
						else
...
(function () {
	...
	function finderTree($filter) {
		return {
			...
			replace: true,
			template: '<div>' +
				'<finder-tree-wrapped data="data" ng-model="dataModel" data-ng-hide="searchFilter" folderimg="folderimg" fileimg="fileimg" data-ng-transclude>' +
				'</finder-tree-wrapped>' +
				...
				scope.$watch('searchFilter', function (newV, oldV) {
					...
					if (angular.isDefined(newV)) {
						if (newV.length > 0) {
							...
							if(scope.data.files && !scope.data.dirs)
								file.itemNumber = scope.data.files.length;
							else if(scope.data.dirs && !scope.data.files)
								file.itemNumber = scope.data.dirs.length;
							else
